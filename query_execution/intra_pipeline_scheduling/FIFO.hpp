...
#ifndef QUICKSTEP_QUERY_EXECUTION_INTRA_PIPELINE_SCHEDULING_FIFO_HPP_
#define QUICKSTEP_QUERY_EXECUTION_INTRA_PIPELINE_SCHEDULING_FIFO_HPP_
...
class FIFO : public IntraPipelineScheduling {
 public:
  FIFO(const QueryExecutionState &query_exec_state,
  ...
  int getNextOperator() override {
    for (auto running_pipeline_iter = current_pipelines_.begin();
         running_pipeline_iter != current_pipelines_.end();
      ...
      for (auto it = all_operators_in_active_pipeline.begin();
           it != all_operators_in_active_pipeline.end();
           ++it) {
...
#endif  // QUICKSTEP_QUERY_EXECUTION_INTRA_PIPELINE_SCHEDULING_FIFO_HPP_
